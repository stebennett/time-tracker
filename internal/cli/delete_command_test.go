...
func TestDeleteCommand_OnlyDeletesSelectedTaskEntries(t *testing.T) {
	...
	// Create tasks
	task1, _ := mockAPI.CreateTask("Task 1")
	task2, _ := mockAPI.CreateTask("Task 2")
	task3, _ := mockAPI.CreateTask("Task 3")
	
	// Create time entries for each task
	now := time.Now()
	entry1, _ := mockAPI.CreateTimeEntry(task1.ID, now.Add(-2*time.Hour), &now)
	entry2, _ := mockAPI.CreateTimeEntry(task2.ID, now.Add(-1*time.Hour), &now)
	entry3, _ := mockAPI.CreateTimeEntry(task3.ID, now.Add(-30*time.Minute), &now)
	
	// Verify initial state
	allEntries, _ := mockAPI.ListTimeEntries()
	assert.Len(t, allEntries, 3, "Should have 3 time entries initially")
	
	allTasks, _ := mockAPI.ListTasks()
...
func TestDeleteCommand_BugScenario(t *testing.T) {
	...
	task1, _ := mockAPI.CreateTask("Domain refactoring test")
	task2, _ := mockAPI.CreateTask("API layer testing")
	task3, _ := mockAPI.CreateTask("CLI command testing")
	
	// Create time entries for each task
	now := time.Now()
	mockAPI.CreateTimeEntry(task1.ID, now.Add(-3*time.Hour), &now)
	mockAPI.CreateTimeEntry(task2.ID, now.Add(-2*time.Hour), &now)
	mockAPI.CreateTimeEntry(task3.ID, now.Add(-1*time.Hour), &now)
	...
	allEntriesBuggy, _ := mockAPI.SearchTimeEntries(domain.SearchOptions{}) // Empty search = all entries
	...
	allEntriesActual, _ := mockAPI.ListTimeEntries()
	...
	task1Entries, _ := mockAPI.SearchTimeEntries(domain.SearchOptions{TaskID: &task1.ID})
	...
	for _, entry := range task1Entries {
		mockAPI.DeleteTimeEntry(entry.ID)
	}
	
	// Delete task 1
	mockAPI.DeleteTask(task1.ID)
	...
	task2Entries, _ := mockAPI.SearchTimeEntries(domain.SearchOptions{TaskID: &task2.ID})
	assert.Len(t, task2Entries, 1, "Task 2 should still have its entry")
	
	task3Entries, _ := mockAPI.SearchTimeEntries(domain.SearchOptions{TaskID: &task3.ID})
